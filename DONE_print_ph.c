...
static	char	*make_x(char b[65], t_flags op)
{
	char *x;
...
void			prints_num2(char b[65], t_flags op)
{
	...
	x = make_x(b, op);
	if (((op.add || op.space) && b[0] != '-' && op.width--)
		|| (b[0] == '-' && b++ && op.precision && len--)
		|| (op.plhld == 'o' && !op.precision && op.width--)
		|| (op.alt && op.plhld != 'o'
...
void			prints_float(char b[65], t_flags op)
{
	...
	if (op.precision)
	{
		round_float(b, int_len + op.width - 1);
		if (op.width)
